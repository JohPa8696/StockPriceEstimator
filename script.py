...
def ParseData(path):
	...
	#Array of the dates at which the stock was priced
	dateColumn = dataframe['Date'].values
	D =  np.zeros(dateColumn.shape)
	#Covert the date to integer value
	for i, j in enumerate(dateColumn):
		D[i] = datetime.strptime(j,'%Y-%m-%d').timestamp()
	dataframe['Timestamp'] = D
	#Remove any unused columns (axis = 1 specifies fields are columns)
	return dataframe.drop('Date', axis = 1)
...
###########AUTHOR: NICHOLAS: https://github.com/nicholastoddsmith/pythonml ############
...
def main():
	...
	R = MLPR(layers, maxIter = 1000, tol = 0.40, reg = 0.001, verbose = True)
	...
	P = sp.PredictDate('2017-06-29', '2017-07-31')
